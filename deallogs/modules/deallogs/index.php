...
$title = Flux::message('AccountDealLogs');
?>
...
foreach ($session->getAthenaServerNames() as $serverName) {
	...
	$sql  = "SELECT `char_id` FROM {$server->charMapDatabase}.char WHERE `account_id` = $accountID";
...
if ($char) {
    ...
    $sql = "SELECT COUNT(id) AS total FROM {$server->logsDatabase}.picklog WHERE type = 'T' AND `char_id` IN (".implode(',', $presti_arr,).")";
    ...
    $sql = $paginator->getSQL("SELECT $col FROM {$server->logsDatabase}.picklog WHERE type = 'T' AND `char_id` IN (".implode(',', $presti_arr,).")");
    $sth = $server->connection->getStatementForLogs($sql);
    $sth->execute();
