...
$searchnameid = isset($_GET['search_nameid']) ? trim($_GET['search_nameid']) : '';
$searchUniqueID = isset($_GET['search_unique_id']) ? trim($_GET['search_unique_id']) : '';
...
if ($searchChar !== '') {
    $searchCondition .= "AND char_id IN (SELECT char_id FROM {$server->charMapDatabase}.`char` WHERE name LIKE ?)";
    $searchParams[] = "%$searchChar%";
}

if ($searchUniqueID !== '') {
    $searchCondition .= " AND unique_id LIKE ?";
    $searchParams[] = "%$searchUniqueID%";
}

if ($searchnameid !== '') {
    $searchCondition .= " AND nameid LIKE ?";
    $searchParams[] = "%$searchnameid%";
}

if ($search_Date !== '') {
    $searchCondition .= " AND time LIKE ?";
    $searchParams[] = "%$search_Date%";
}

$sql = "SELECT COUNT(id) AS total FROM {$server->logsDatabase}.picklog WHERE type = 'T' $searchCondition";
$sth = $server->connection->getStatementForLogs($sql);
$sth->execute($searchParams);
...
$col = "time, char_id, type, nameid, amount, refine, card0, card1, card2, card3, option_id0, option_val0, map, unique_id";
$sql = $paginator->getSQL("SELECT $col FROM {$server->logsDatabase}.picklog WHERE type = 'T' $searchCondition");
$sth = $server->connection->getStatementForLogs($sql);
$sth->execute($searchParams);


if ($searchChar !== '') {
    $sth->execute(array("%$searchChar%"));
} else {
    $sth->execute();
}
