...
class Feed
{
	...
	/** @var string */
	public static $cacheDir = __DIR__.'/fcache';
	...
	{
		if (extension_loaded('curl') && Feed::$useCurl) {
			$curl = curl_init();
			...
			return curl_errno($curl) === 0 && curl_getinfo($curl, CURLINFO_HTTP_CODE) === 200
				? $result
				: Feed::httpRequest_sub($url, $user, $pass);
	...
	private static function httpRequest_sub($url, $user, $pass) {
		$context = null;
		if ($user !== null && $pass !== null) {
			$options = [
				'http' => [
...
try {
	$limit = 5;
	$cid = array( 0 => 'news', 1 => 'events', 2 => 'updates');
	$c = array();

	foreach($config['news_feed'] as $k1 => $feed) {
		if(strtolower($feed['feed_type']) == 'atom') {
			$data = Feed::loadAtom($feed['feed_url']);
			foreach($data->entry as $k2 => $item) {
				if(! isset($c[$k1])) $c[$k1] = 0;
				if($c[$k1] >= $limit) continue;
				$feed_data[] = array(
					'css' => $cid[$k1],
					'cat_name' => $feed['category_name'],
					'title' => htmlspecialchars($item->title),
					'link' => htmlspecialchars($item->link['href']),
					'date' => date(Flux::config('DateFormat'), (int) $item->timestamp),
					'desc' => ($item->content['feed_type'] == 'html' ? strip_tags($item->content) : htmlspecialchars(strip_tags($item->content)) )
				);
				$c[$k1]++;
			}

		} else if (strtolower($feed['feed_type']) == 'rss') {
			$data = Feed::loadRss($feed['feed_url']);
			foreach($data->item as $k2 => $item) {
				if(! isset($c[$k1])) $c[$k1] = 0;
				if($c[$k1] >= $limit) continue;
				$feed_data[] = array(
					'css' => $cid[$k1],
					...
					'date' => date(Flux::config('DateFormat'), (int) $item->timestamp),
					'desc' => (isset($item->{'content:encoded'}) ? strip_tags($item->{'content:encoded'}) : htmlspecialchars(strip_tags($item->description)) )
				);
				$c[$k1]++;
			}

		} else {
			throw new Exception( "Feed type does not exists" );
		}
	}
	usort($feed_data, 'date_compare');

} catch (Exception $e){
	// throw new Flux_Error("RSS THEME ERROR! ".$e->getMessage());
